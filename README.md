...
> 📄 **AI 产品交付物**：[PRD](docs/ai-pm/PRD.md) · [受控工作流](docs/ai-pm/CONTROLLED_WORKFLOW.md) · [评测与 Bad Case](docs/ai-pm/EVALUATION_AND_BAD_CASES.md)
...
│   ├── copilot_api.py       # ⑤ 本地案件查询 / 人工审批 API（不执行资金动作）
│   └── evaluate_copilot.py  # ⑥ 金标回归 + Bad Case 安全兜底评测
├── sql/
...
3. **可选 LLM 摘要**：加 `--llm` 且配置 `ANTHROPIC_API_KEY` 时，对 Top-N 条把已验证证据交给 Claude 生成自然语言研判（无密钥自动跳过）。模型不得改变金额、规则裁定或执行资金动作。
4. **人工审批与审计**：`POST /cases/{case_id}/human-decision` 只记录采纳、驳回或升级决定；API 没有支付、退款、追回或催票端点。
...
## 简历 bullet（示意）
> **物流结算异常复核 Copilot（个人 AI 产品项目）** — 基于 Olist 10 万+ 电商订单，用 **DuckDB SQL** 构建
...
> 生成坏账候选与按客户应收暴露报表；并构建**异常金额—单据溯源 + 受控复核 Copilot**（可选 LLM 仅负责证据摘要），
> 生成拒付/追回/催票的**人工审批建议**；在模拟账单中识别潜在可追回金额约 14 万，并实现对账、复核与报表的一键复现。
